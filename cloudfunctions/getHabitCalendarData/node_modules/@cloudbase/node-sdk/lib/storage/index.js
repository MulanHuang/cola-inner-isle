...
/*
 * 上� 文件
 * @param {string} cloudPath 上� 后的文件路径
 * @param {fs.ReadStream} fileContent  上� 文件的二进制流
 */
...
async function deleteFile(cloudbase, { fileList }, opts) {
    ...
    for (let file of fileList) {
        if (!file || typeof file !== 'string') {
            return utils_1.processReturn(cloudbase.config.throwOnCode, Object.assign({}, code_1.ERROR.INVALID_PARAM, { message: 'fileList的元� 必须是非空的字符串' }));
        }
    }
    let params = {
    ...
    for (let file of fileList) {
        if (typeof file === 'object') {
            if (!file.hasOwnProperty('fileID') || !file.hasOwnProperty('maxAge')) {
                return utils_1.processReturn(cloudbase.config.throwOnCode, Object.assign({}, code_1.ERROR.INVALID_PARAM, { message: 'fileList的元� 如果是对象，必须是包含fileID和maxAge的对象' }));
            }
            file_list.push({
                fileid: file.fileID,
                max_age: file.maxAge
            ...
            return utils_1.processReturn(cloudbase.config.throwOnCode, Object.assign({}, code_1.ERROR.INVALID_PARAM, { message: 'fileList的元� 如果不是对象，则必须是字符串' }));
        }
    }
    let params = {
